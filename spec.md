...
* **A. 광학 산란형 에너지 배리어 (Refractive Energy Barrier - 광학/빔 방어용):**
  * **과학적 고증 및 원리:** 고밀도 초전도 자기장 루프로 구속된 이온화 플라즈마 가스층을 전개합니다. 입사되는 레이저나 플라즈마 포탄 등 고주파 전자기 빔이 배리어에 닿는 순간, 빛의 굴절율을 극한으로 왜곡하여 에너지를 사방으로 **'산란(Dispersion)'**시키거나 상쇄 위상 간섭을 일으켜 에너지를 감쇄시킵니다.
  * **인게임 연출:** 레이저나 플라즈마 공격을 받으면 파란색 육각형 벌집 문양의 보호막 배리어가 펼쳐지며 빔이 굴절되어 빗겨나가는 광학 이펙트가 발생합니다.
  * **성능 스펙:** 외계인의 **에너지 계열 공격** 피해량을 80% 흡수(차단)하지만, 실물 키네틱 탄환에는 산란막이 통째로 찢기며 거의 방어 효과가 없습니다.

* **B. 능동 요격형 키네틱 디펜더 (Active Kinetic Interceptor - 실물 탄환/미사일 방어용):**
  * **과학적 고증 및 원리:** 실물 철갑탄(레일건 탄두)이나 미사일의 물리적 질량 충격을 막아내는 방어막입니다.
    1. **자기장 척력 편향 (Electromagnetic Deflection):** 자성을 띠는 레일건 탄환이 접근하면 궤도 기지에서 초고출력 척력 자기장을 유도하여 탄환의 비행 궤적을 튕겨내거나 흐트러뜨립니다.
    2. **능동 SAM 파편 요격 (Active Intercept System):** 대형 철갑 미사일이 감지되면 요새의 미세 요격 포대에서 초소형 파편 자탄(SAM/CIWS 방식)을 역사격하여 탄두가 기지에 닿기 전 대기권 외곽에서 조기 폭발시킵니다.
  * **인게임 연출:** 적의 레일건 탄환이 기지 주변에서 튕겨 나가거나, 미사일이 날아올 때 기지에서 소형 요격 빔/자탄이 발사되어 공중 폭발하는 연출이 가시화됩니다.
  * **성능 스펙:** 외계인의 **실물/탄환 계열 공격** 피해량을 80% 흡수하며, 에너지 빔 공격에 대해서는 요격 미사일이나 자기장 척력이 작동하지 않으므로 효과가 무효화됩니다.

* **실드 vs 아머 공격 상성 테이블 (Combat Mechanics):**
  전투 유닛 및 방어 포탑은 공격 방식에 따라 실드와 아머에 가하는 데미지 배율이 완전히 달라집니다.

  | 공격 속성 계열 | 광학 에너지 배리어 피해 | 키네틱 디펜더 피해 | 아머/체력(Armor/HP) 피해 | 대표적인 아군 무기 및 유닛 |
  | :--- | :---: | :---: | :---: | :--- |
  | **에너지 계열 (Energy)** | **$150\%$ (실드 조기 흡수)** | $10\%$ (거의 차단 못함) | $50\%$ (아머에 반감 피해) | 플라즈마 레이저 캐논, 타겟팅 레이저 위성 |
  | **실물/탄환 계열 (Kinetic)** | $10\%$ (거의 차단 못함) | **$150\%$ (요격 탄약 급소모)** | **$150\%$ (기갑 관통 파괴)** | 대함 레일건 요새, 개틀링, 쉽야드 구축함 주포 |
  | **EMP/전술 계열 (EMP)** | **즉시 실드 완전 셧다운** | 무력화 (작동 정지 5초) | 피해 없음 (3초 마비) | 기가 플라즈마 요새포 (액티브 스킬 포함) |

* **실드 복구 및 가동 (Shield Management):**
  * 두 종류의 실드(광학 배리어 / 키네틱 디펜더)는 건설 완료 시 추가적인 지속 크레딧 소모 없이 **글로벌 에너지(Energy) 풀의 전력을 나누어 소모**하며 상시 가동됩니다.
  * 포격으로 손상된 실드 에너지는 에너지가 충분할 때 초당 2%씩 자동으로 충전 및 회복되므로, 별도의 번잡한 탄약 구매나 수동 유지보수 조작이 필요 없습니다. (방치형 플레이 유지)
