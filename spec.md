...
행성 방어망은 **지상 방어 기지**, **궤도 방어 위성**, **궤도 방어 기지**, **궤도 쉽야드 및 방어부대**, 그리고 **행성 실드 발생기**의 5중 구조로 다채롭게 설계하여 전투의 입체감을 높입니다.

#### 1) 지상 방어 기지 (Ground Defense Bases)
* **개요:** 행성 지표면에 건설하는 고정형 방어 거점. 기존 방패와 연계해 키네틱 요격율을 상승시킵니다.
* **건설 제한 (수량 한도):** 기존의 행성당 통합 8개 한도가 폐지되고, **기지 종류별로 각각 독립적인 건설 수량 한도(기본 최대 8개)**가 적용됩니다. 각 기지 카드는 현재 수량 및 최대 한도(예: `0 / 8개`)를 UI 상에 정확히 시각화하여 표현합니다.
* **건설 실패 피드백 세분화:** 자원 부족(크레딧/에너지) 시에도 최대 한도에 도달했다고 안내하던 모호한 팝업을 개편하여, 실패 사유(최대 건설 한도 도달, 크레딧 부족, 가용 전력 부족)를 명확하게 분리하여 사용자에게 피드백합니다.
* **공격 무기:**
  * **대함 레일건 요새:** 200 HP 데미지, 5 초 쿨다운, 적 함선 직접 타격, 장갑 관통.
  * **지대공 에너지 캐논:** 80 HP 데미지, 2 초 쿨다운, 공중 유닛 및 미사일 요격.
  * **지상 요격 미사일 사일로:** 120 HP 데미지, 3 초 쿨다운, 저고도·지상 접근 적 타격.
  * **플라즈마 폭격기:** 250 HP 데미지, 7 초 쿨다운, AoE 광역 폭발.
  * **전기 방출 포탑:** 150 HP 데미지, 4 초 쿨다운, 전자기 방해·이동 속도 감소.
  * **초전도 스나이퍼 캐논:** 350 HP 데미지, 10 초 쿨다운, 초장거리 단일 타겟, 보스 특화.
  * **속사 개틀링 포탑:** 30 HP 데미지, 0.4 초 쿨다운, 드론·소형 유닛 난사.
  * **핵 어뢰 사일로:** 800 HP 데미지, 30 초 쿨다운, AoE 대형, 크레딧 소모.
* **방어 모듈:**
  * **지상 포스필드 배리어:** 기지 HP +30% 실드 추가, 상시 에너지 소모.
  * **미사일 방어막 (CIWS):** 적 미사일 자동 요격, 0.8 초 쿨다운.
  * **강화 장갑 플레이팅:** 수신 데미지 25% 감소, 패시브.
* **장단점:** 튼튼하지만 대기권 굴절 및 사거리 제한이 있음.

#### 2) 궤도 방어 위성 (Orbital Defense Satellites)
* **개요:** 행성 주변 궤도를 끊임없이 공전하는 무인 위성 형태의 소형 방어 장치이며, 레이저 무기로 적을 직접 타격합니다.
* **시각화 (공전 렌더링 및 공격 발사):** 건설된 궤도 위성은 행성(지구) 주변 궤도 상에 실시간으로 렌더링되며, 궤도 패스 라인을 따라 60FPS로 끊임없이 공전하는 연출이 시각화됩니다. 위성 본체와 좌우 태양광 패널 구조물 형태를 가집니다. 위성에서 발사되는 모든 공격 투사체(에너지 레이저 등)는 궤도 상에 공전하는 개별 위성의 실시간 X, Y 위치에서 직접 발원하여 타겟 적 함선의 위치로 발사되도록 설계하여 발사 지점의 시각적 일관성을 확보합니다.
