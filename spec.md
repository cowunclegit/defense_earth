...
#### 5) 아군 및 적군 에너지 실드 메커니즘 (Energy Shield System)
전투의 전략적 상성과 손맛을 극대화하기 위해 아군 행성 방어망과 적 외계 함선 모두에게 공통 적용되는 이중 방어(실드 & 아머) 시스템을 적용합니다.

* **아군 광역 Aegis 실드 작동 방식:**
  * **쉴드 전개:** 지상의 '에너지 실드 제너레이터'나 '궤도 방어 기지'를 가동하면 행성 및 주변 궤도 전체를 감싸는 초대형 보호막인 **Aegis 실드**가 전개됩니다.
  * **자원 요구:** Aegis 실드를 유지하고 복구하기 위해서는 글로벌 에너지 풀(Energy Pool) 중 일정 수치(예: 발전소 전체 출력의 30%)를 상시 '쉴드 충전용'으로 소모해야 합니다.
  * **실드 붕괴 패널티:** 실드 에너지(Shield HP)가 적의 집중 포격을 받아 0이 되면 실드가 꺼지며(오프라인), 이 상태에서 지상/궤도 건물이 타격을 받으면 수리비가 발생하고 거주 인구(세금 수입처)가 격감하는 피해를 입습니다.
  * **충전 및 시너지:** 비전투 상태에서 초당 2%씩 자연 재생되며, **달의 테라포밍 버프(재생률 +25%)**나 **시공 영구 업그레이드(쉴드 증가)** 혜택을 직접 적용받습니다.

* **실드 vs 아머 공격 상성 테이블 (Combat Mechanics):**
  전투 유닛 및 방어 포탑은 공격 방식에 따라 실드와 아머에 가하는 데미지 배율이 완전히 달라집니다.

  | 공격 속성 계열 | 실드(Shield) 피해 | 아머/체력(Armor/HP) 피해 | 대표적인 아군 무기 및 유닛 |
  | :--- | :---: | :---: | :--- |
  | **에너지 계열 (Energy)** | **$150\%$ (추가 피해)** | $50\%$ (반감 피해) | 플라즈마 레이저 캐논, 타겟팅 레이저 위성 |
  | **실물/탄환 계열 (Kinetic)** | $50\%$ (반감 피해) | **$150\%$ (추가 피해)** | 대함 레일건 요새, 개틀링, 쉽야드 구축함 주포 |
  | **EMP/전술 계열 (EMP)** | **즉시 실드 완전 파괴** | 피해 없음 (3초 마비) | 기가 플라즈마 요새포 (액티브 스킬 포함) |

* **실드 방어 전술 시너지:**
  * 플레이어는 적의 선두에서 진입하는 실드 디스럽터나 바이패서 함선을 에너지 계열(레이저 위성 등)로 먼저 실드를 빠르게 녹여내고, 실드가 벗겨진 적 본체들을 지상의 고화력 대함 레일건으로 관통 격추하는 형태의 순차적 사격 타겟팅을 수행하게 됩니다.
