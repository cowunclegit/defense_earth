...
* **A. 흡수형 에너지 실드 (Absorptive Energy Shield - 광학/에너지 방어용):**
  ...
  * **성능 스펙:** 외계인의 **에너지 계열 공격** 피해량을 100% 완벽하게 흡수하여 지상 기지를 지켜내며, 특수 연구 완료 시 피격 에너지의 $10\%$를 글로벌 전력(Energy)으로 실시간 역전환하여 재자원화합니다. (실물 탄환에는 전혀 반응하지 않습니다)

* **B. 탄막 요격형 키네틱 실드 (Flak/SAM Kinetic Shield - 실물 탄환/미사일 방어용):**
  ...
  * **확률적 요격 기믹 (Flak Probability):** 절대 차단 장벽이 아닌, 실제 대공 방어망처럼 **확률적 요격 시스템**으로 작동합니다. 적의 포탄이 너무 과도하게 밀려들 경우 요격 한계에 도달해 일부 탄탄 및 미사일을 놓쳐 지상이나 궤도 기지에 직접적인 피해를 허용할 수 있습니다.
  * **성능 향상 조건:** 요격소의 배치 개수가 많거나, 과학소의 연구 및 기지 레벨이 높아질수록 요격탄 사출 속도가 비약적으로 향상되어 최종 요격 성공률(기본 $60\%$ ➔ 최대 $98\%$)이 완벽에 가깝게 증대됩니다.
  * **인게임 연출:** 미사일이나 탄환이 날아올 때 궤도 요새에서 소형 자탄이 쏟아져 나가며 궤도 상에서 화려한 폭발(Flak Burst)을 일으켜 투사체를 요격하는 전투 연출이 시각화됩니다.
...
* **실드 vs 아머 공격 상성 테이블 (Combat Mechanics):**
  ...
  | :--- | :---: | :---: | :---: | :--- |
  | **에너지 계열 (Energy)** | **$150\%$ (에너지 흡수)** | $0\%$ (요격 불가) | $50\%$ (아머에 반감 피해) | 플라즈마 레이저 캐논, 타겟팅 레이저 위성 |
  | **실물/탄환 계열 (Kinetic)** | $0\%$ (전혀 흡수 안됨) | **$60\%\sim98\%$ (대공 역사격 격추)** | **$150\%$ (기갑 관통 파괴)** | 대함 레일건 요새, 개틀링, 쉽야드 구축함 주포 |
