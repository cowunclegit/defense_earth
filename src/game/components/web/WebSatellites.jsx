  ...
  return (
    <>
      ...
      {processedSatellites.map((sat, index) => {
        ...
        const solarColor = isOnline ? '#00f0ff' : '#555555';
        const solarOpacity = isOnline ? 0.8 : 0.3;
        ...
        return (
          <g key={`sat-web-${index}`} transform={`translate(${satX}, ${satY}) rotate(${angleDeg})`}>
            {/* Solar panels */}
            <rect x={-9} y={-1.5} width={6} height={3} fill={solarColor} stroke="#050814" strokeWidth={0.5} opacity={solarOpacity} />
            <rect x={3} y={-1.5} width={6} height={3} fill={solarColor} stroke="#050814" strokeWidth={0.5} opacity={solarOpacity} />
            {/* Satellite Body */}
            <rect x={-3} y={-3} width={6} height={6} fill={bodyColor} stroke="#050814" strokeWidth={1} opacity={bodyOpacity} />
          </g>
        );
      })}
    </>
